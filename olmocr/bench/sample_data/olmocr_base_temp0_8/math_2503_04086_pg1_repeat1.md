...
\[
\lambda_g = \sum_{s \in S} \sum_{i=1}^{k} \sum_{r_s=I_i} \zeta_n^{\psi(gs)} = \sum_{s \in S} \sum_{i=1}^{k} \sum_{r_s=I_i} \zeta_n^{\psi(gs)}.
\]
...
\[
\sum_{s, r_s=I_i} \zeta_n^{\psi(gs)} = \sum_{u \in (R/Ann_R(x_i))^\times} \zeta_n^{\psi(gux_i)} = \sum_{u \in (R/Ann_R(x_i))^\times} \zeta_n^{\psi_i(gu)} = c(g, R/Ann_R(x_i)).
\]
