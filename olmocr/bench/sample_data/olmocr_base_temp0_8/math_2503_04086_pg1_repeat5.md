...
$$\lambda_g = \sum_{s \in S} \zeta^n(g^s) = \sum_{i=1}^{k} \left[ \sum_{s, Rs = I_i} \zeta^n(g^s) \right].$$
...
$$\sum_{s, Rs = I_i} \zeta^n(g^s) = \sum_{u \in (R/\text{Ann}_R(x_i))^\times} \zeta^n(gux_i) = \sum_{u \in (R/\text{Ann}_R(x_i))^\times} \zeta^n(gu) = c(g, R/\text{Ann}_R(x_i)).$$
