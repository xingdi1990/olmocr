...
\[
\lambda_g = \sum_{s \in S} \zeta_n^g(s) = \sum_{i=1}^{k} \left[ \sum_{s, Rs = I_i} \zeta_n^g(s) \right].
\]
...
\[
\sum_{s, Rs = I_i} \zeta_n^g(s) = \sum_{u \in (R/\Ann_R(x_i))^\times} \sum_{s, Rs = I_i} \zeta_n^g(u s) = \sum_{u \in (R/\Ann_R(x_i))^\times} \zeta_n^{\psi_x}(gu) = c(g, R/\Ann_R(x_i)).
\]
Here we recall that $\psi_x$ is the induced linear functional on $R/\Ann_R(x_i)$. We conclude that \( \lambda_g = \sum_{i=1}^{k} c(g, R/\Ann_R(x_i)) \).
