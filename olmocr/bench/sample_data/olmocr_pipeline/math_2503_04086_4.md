...
\[
\lambda_g = \sum_{s \in S} \zeta_i^{g(s)} = \sum_{i=1}^{k} \left[ \sum_{s, Rs = I_i} \zeta_i^{g(s)} \right].
\]
...
\[
\sum_{s, Rs = I_i} \zeta_i^{g(s)} = \sum_{u \in (R/\text{Ann}_R(x_i))^\times} \zeta_i^{g(u)x_i} = \sum_{u \in (R/\text{Ann}_R(x_i))^\times} \zeta_i^{\psi_{x_i}(gu)} = c(g, R/\text{Ann}_R(x_i)).
\]
Here we recall that $\psi_{x_i}$ is the induced linear functional on $R/\text{Ann}_R(x_i)$. We conclude that $\lambda_g = \sum_{i=1}^{k} c(g, R/\text{Ann}_R(x_i))$. □
