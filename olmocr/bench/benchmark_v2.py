...
def evaluate_candidate(
    ...
    if candidate_errors and not force:
    ...
    if force:
        def process_test(test: BasePDFTest) -> Tuple[float, str, str, List[str], Tuple[bool, str], bool]:
            local_errors = []
            test_failure = None
            pdf_name = test.pdf
            ...
            if not page_md_files:
                # When force=True, skip this test instead of treating it as an error
                return (0.0, None, test.type, [], (False, "Missing MD files"), True)  # True indicates skip
            ...
            return (test_avg, test_failure, test.type, local_errors, (final_passed, final_explanation), False)
    else:
    ...
    with ThreadPoolExecutor(max_workers=min(os.cpu_count() or 1, 64)) as executor:
        ...
        if force:
            valid_tests_count = 0  # Track how many tests actually ran (not skipped)
            for future in tqdm(as_completed(futures), total=len(futures), desc=f"Evaluating tests for {candidate_name}", unit="test"):
                test_avg, test_failure, test_type, errors, _, should_skip = future.result()
                if should_skip:
                    # Skip this test when force=True and it has missing MD files
                    continue
            ...
            overall_score = total_test_score / valid_tests_count if valid_tests_count > 0 else 0.0
        else:
            # Original behavior when force=False
            for future in tqdm(as_completed(futures), total=len(futures), desc=f"Evaluating tests for {candidate_name}", unit="test"):
...
def main():
    ...
    for candidate in candidate_folders:
        ...
        if candidate_errors and not args.force:
