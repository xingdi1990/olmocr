...
async def run_olmocr(pdf_path: str, page_num: int = 1, temperature: float = 0.8) -> str:
    ...
    try:
        # Process the page
        query = await build_page_query(pdf_path, page_num, args.target_longest_image_dim, args.target_anchor_text_len)
        ...
        # Update metrics
        metrics.add_metrics(
            sglang_input_tokens=response_data["usage"].get("prompt_tokens", 0), sglang_output_tokens=response_data["usage"].get("completion_tokens", 0)
        )
